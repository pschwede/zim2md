...
def translate(text: List[str], path:str="", nbpath:str="") -> List[str]:
    ...
    for i, line in enumerate(text):
        if line.startswith("======") and headline_nr == 0:
            headline_nr = i
            ...
            text[i] = line
            continue
        if line.startswith("Created") and headline_nr == i-1:
            line = sub(r"^Created", "created_at:: ", line)
            text[i] = line
            continue
        if headline_nr == i-2 and relpath:
            line = "[[" + relpath + "]]\n" + line
        ...
        # Lists
        line = sub(r"^(\s*)\[\*\]", r"\g<1>- DONE", line, count=1)
        line = sub(r"^(\s*)\[x\]", r"\g<1>- CANCELED", line, count=1)
        line = sub(r"^(\s*)\[>\]", r"\g<1>- DOING", line, count=1)
        line = sub(r"^(\s*)\[ \]", r"\g<1>- TODO", line, count=1)
        ...
        # TODO Images
        line = sub(r"{{(.+?)|(.+?)}}", r"[[\g<2>]["+relpath+r"\g<1>]]", line)
        line = sub(r"{{(.+?)}}", r"[[\g<1>]["+relpath+r"\g<1>]]", line)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) <= 1:
        ...
        _newpath = os.path.join(sys.argv[2], "pages")
        _path = sys.argv[1]
        ...
        for _file in Path(_path).rglob("*.txt"):
            with open(_file, 'r') as _f:
                lines = _f.readlines()
                outpath = str(os.path.relpath(_file, _path).replace(os.sep, ""))
